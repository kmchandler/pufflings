...
describe('Child lib funtions', () => {
  ...
  describe('createChild', () => {
    it('should throw for no current user', async () => {
      ...
      expect(async () => await createChild(new FormData())).rejects.toThrow(
        'no user'
      );
    });

    it('should look for the family with the user id', async () => {
      (prisma.family_user.findFirstOrThrow as jest.Mock).mockReturnValue(
